    ...
    def post(self, request):
        ...
        if form.is_valid() and formset.is_valid():
            ...
            for form in formset:                                                # for loop to save each individual form as its own object
                ...
                billitem.billno = billobj                                       # links the bill object to the items
                # gets the stock item
                selling_item = Stock.objects.get(code = form.cleaned_data['code'])
                print(selling_item.pk)
                billitem.stock = selling_item
                billitem.perprice = selling_item.sell_price
...
def show_product(request):
    if request.method == 'GET':
        ...
        print(code)
        product = Stock.objects.get(code = code) #getting the liked posts
        prod = {}
        prod['item'] = product.name
        prod['price'] = product.sell_price
        prod['available'] = product.quantity
        prod['status_code'] = 200
